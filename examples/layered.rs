use bsp_pathfinding::{
    ...
    heuristics, BSPNode, BSPTree, Face, LayeredNavigationContext, Portal, Portals, Shape,
};
...
async fn main() {
    let scenes = [spawn_scene_1(), spawn_scene_2()]
        .into_iter()
        .enumerate()
        .flat_map(|(i, v)| v.into_iter().map(move |v| (i as f32, v)))
        .collect_vec();

    let mut nav = LayeredNavigationContext::new(scenes.iter().cloned());
    ...
    loop {
        ...
        if let Some(c) = get_char_pressed() {
            match c {
                ...
                '-' => layer -= 0.1,
                'r' => {
                    nav = LayeredNavigationContext::new(scenes.iter().cloned());
                    seed += 1
                }
                'R' if seed > 0 => {
                    nav = LayeredNavigationContext::new(scenes.iter().cloned());
        ...
        start += nav.locate(layer, start).unwrap().depth;
        ...
        let (_, tree, portals) = nav.layer(layer).unwrap();
        tree.descendants()
